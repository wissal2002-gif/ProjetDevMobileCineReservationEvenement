...
class AdminEndpoint extends Endpoint {
  ...
  Future<Map<String, int>> getDashboardActions(Session session) async {
    final user = await _getRequiredUser(session);
    final pendingSupport = await DemandeSupport.db.count(session,
        where: (t) => t.statut.notEquals('traité'));
    int cancelledRes = 0;
    if (user.role == 'super_admin' || user.role == 'resp_evenements') {
      cancelledRes = await Reservation.db.count(session,
          where: (t) => t.statut.equals('annule'));
      ...
      if (sIds.isNotEmpty) {
        final seances = await Seance.db.find(session,
            where: (t) => t.salleId.inSet(sIds));
        final seanceIds = seances.map((s) => s.id!).toSet();
        if (seanceIds.isNotEmpty) {
          cancelledRes = await Reservation.db.count(session,
              where: (t) =>
              (t.seanceId.inSet(seanceIds)) & t.statut.equals('annule'));
        }
      }
    }
    return {
  ...
  // ✅ MODIFIÉ : resp_evenements a son propre bloc
  Future<Map<String, int>> getAdminStats(Session session) async {
    final user = await _getRequiredUser(session);

    // ✅ APRÈS : super_admin seul
    ...
    // ✅ NOUVEAU : bloc resp_evenements
    if (user.role == 'resp_evenements' && user.cinemaId != null) {
      final cinema = await Cinema.db.findById(session, user.cinemaId!);
      final eventCount = await Evenement.db.count(session,
        where: (e) => e.cinemaId.equals(user.cinemaId) |
        e.ville.equals(cinema?.ville ?? ''),
      );
      final resCount = await Reservation.db.count(session,
        where: (r) => r.evenementId.notEquals(null),
      );
      return {
        'totalFilms': 0,
    ...
    if (sIds.isNotEmpty) {
      ...
      final seanceIds = seances.map((s) => s.id!).toSet();
      if (seanceIds.isNotEmpty) {
        resCount = await Reservation.db.count(session,
            where: (t) => t.seanceId.inSet(seanceIds));
      }
    }
    final eventCount = await Evenement.db.count(session,
        where: (t) => t.cinemaId.equals(user.cinemaId));
    final userCount = await Utilisateur.db.count(session,
        where: (t) => t.cinemaId.equals(user.cinemaId));
    int filmCount = 0;
    try {
  ...
  Future<List<Reservation>> getAllReservations(Session session) async {
    final user = await _getRequiredUser(session);
    if (user.role == 'super_admin') return await Reservation.db.find(session,
        orderBy: (t) => t.dateReservation, orderDescending: true);
    if (user.role == 'resp_evenements') return await Reservation.db.find(session,
        where: (t) => t.evenementId.notEquals(null),
        orderBy: (t) => t.dateReservation, orderDescending: true);
  ...
  Future<void> rembourserReservation(Session session, int resId, double amt, String raison) async {
    ...
    if (res != null) {
      ...
      try {
        ...
        session.log("Erreur envoi email remboursement: $e");
      }
    }
  }
  ...
  Future<void> genererSiegesAutomatique(Session session,
      {required int salleId,
        ...
        required int nbColonnes}) async =>
      await genererSiegesPourSalle(session, salleId, nbRangees, nbColonnes);

  Future<void> updateSiegesType(Session session, List<int> siegeIds, String type) async {
    for (final id in siegeIds) {
      final siege = await Siege.db.findById(session, id);
      if (siege != null) {
        await Siege.db.updateRow(session, siege.copyWith(type: type));
      }
    }
  }

  Future<List<Siege>> getSiegesByReservation(Session session, int resId) async {
    final relations = await ReservationSiege.db.find(session,
        where: (t) => t.reservationId.equals(resId));
  ...
  Future<List<Seance>> getAllSeances(Session session) async =>
      await Seance.db.find(session, orderBy: (t) => t.dateHeure);
  ...
  // --- ÉVÉNEMENTS ---
  // ✅ MODIFIÉ : filtrage selon le rôle
  Future<List<Evenement>> getAllEvenements(Session session) async {
    final user = await _getRequiredUser(session);
    if (user.role == 'super_admin') {
      return await Evenement.db.find(session);
    }
    if (user.role == 'resp_evenements' && user.cinemaId != null) {
      final cinema = await Cinema.db.findById(session, user.cinemaId!);
  ...
  Future<List<Reservation>> getHistoriqueUtilisateur(Session session, int userId) async =>
      await Reservation.db.find(session,
          where: (t) => t.utilisateurId.equals(userId));

  // ✅ MODIFIÉ : ajout du paramètre cinemaId
  Future<void> modifierUtilisateurRole(Session session, int userId, String newRole, int? cinemaId) async {
    final u = await Utilisateur.db.findById(session, userId);
    if (u != null) {
      u.role = newRole;
      u.cinemaId = cinemaId; // ✅ AJOUTÉ
      await Utilisateur.db.updateRow(session, u);
    }
  }
  ...
  // --- FAQ ---
  Future<List<Faq>> getAdminFaqs(Session session) async =>
      await Faq.db.find(session);

  Future<Faq> ajouterFaq(Session session, Faq f) async =>
      await Faq.db.insertRow(session, f);

  Future<Faq> modifierFaq(Session session, Faq f) async =>
      await Faq.db.updateRow(session, f);

  Future<void> supprimerFaq(Session session, int id) async =>
      await Faq.db.deleteWhere(session, where: (t) => t.id.equals(id));
  ...
  Future<OptionSupplementaire> ajouterOption(Session session, OptionSupplementaire o) async =>
      await OptionSupplementaire.db.insertRow(session, o);

  Future<OptionSupplementaire> modifierOption(Session session, OptionSupplementaire o) async =>
      await OptionSupplementaire.db.updateRow(session, o);

  Future<void> supprimerOption(Session session, int id) async =>
      await OptionSupplementaire.db.deleteWhere(session,
          where: (t) => t.id.equals(id));

  Future<String> uploadOptionImage(Session session, List<int> bytes, String fileName) async {
    ...
    final url = await session.storage.getPublicUrl(storageId: 'public', path: path);
    return url?.toString() ?? '';
  }

  // --- CODES PROMO ---
  Future<List<CodePromo>> getAllCodesPromo(Session session) async =>
      await CodePromo.db.find(session);

  Future<CodePromo> ajouterCodePromo(Session session, CodePromo c) async =>
      await CodePromo.db.insertRow(session, c);

  Future<CodePromo> modifierCodePromo(Session session, CodePromo c) async =>
      await CodePromo.db.updateRow(session, c);
  ...
  Future<String> getGlobalPromoSummary(Session session) async {
    ...
    p.actif == true &&
        (p.dateExpiration == null || p.dateExpiration!.isAfter(now))).length;
  ...
  Future<List<Map<String, dynamic>>> getReservationsDetailed(Session session) async {
    ...
    return list.map((r) => {
      ...
      'dateReservation': r.dateReservation.toIso8601String()
    }).toList();
  }

  Future<void> traiterRemboursement(Session session, int resId) async =>
      await rembourserReservation(session, resId, 0, "Remboursement traité");
  ...
  Future<String> getStatsFavoris(Session session) async {
    ...
    final favsFilmsCinema = tousLesFavorisFilms
        .where((f) => filmIds.contains(f.filmId)).toList();
  ...
  Future<String> getAvisFilmsCinema(Session session) async {
    ...
    final avis = await Avis.db.find(session,
        ...
        orderBy: (a) => a.dateAvis, orderDescending: true);
    final userIds = avis.map((a) => a.utilisateurId).toSet();
    final utilisateurs = userIds.isEmpty
        ? <Utilisateur>[]
        : await Utilisateur.db.find(session,
  ...
  Future<String> getStatistiquesDetaillees(Session session) async {
    ...
    for (final salle in salles) {
      ...
      final resIds = reservationsSalle.map((r) => r.id!).toSet();
      int siegesOccupes = 0;
    ...
    for (final film in films) {
      final seancesFilm = seances.where((s) => s.filmId == film.id).toList();
      final seanceFilmIds = seancesFilm.map((s) => s.id!).toSet();
      int totalRes = 0;
      double revenu = 0;
      if (seanceFilmIds.isNotEmpty) {
        ...
        revenu = reservations.fold(0.0, (sum, r) => sum + r.montantTotal);
      }
      statsParFilm.add({
        'filmTitre': film.titre,
    ...
    final resAujourdHui = await Reservation.db.find(session,
        where: (r) => r.cinemaId.equals(cinemaId) &
        r.dateReservation.between(debutJour, finJour));
    final resIdsAujourdHui = resAujourdHui.map((r) => r.id!).toSet();
    ...
    if (resIdsAujourdHui.isNotEmpty) {
      final resOptions = await ReservationOption.db.find(session,
          where: (ro) => ro.reservationId.inSet(resIdsAujourdHui));
      final optionIds = resOptions.map((ro) => ro.optionId).toSet();
      ...
      for (final ro in resOptions) {
        ...
        revenuParOption[ro.optionId] = (revenuParOption[ro.optionId] ?? 0) +
